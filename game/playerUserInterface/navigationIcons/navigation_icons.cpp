...
NavigationIcons::NavigationIcons(const sf::Vector2f& camera_size, const sf::Vector2f& camera_coordinates) {
    ...
    setObjectPosition(camera_size, camera_coordinates);
}

//This setting allows you to adjust to the size of the window and display sprite correctly
void NavigationIcons::setObjectPosition(const sf::Vector2f& camera_size, const sf::Vector2f& camera_coordinates) {
    ...
    menu_icon_.setPosition(camera_coordinates.x + static_cast<float>(camera_size.x) / 2.0f, 
            camera_coordinates.y + static_cast<float>(camera_size.y) - menu_icon_.getGlobalBounds().height - static_cast<float>(camera_size.y) * 1.0f / 100.0f );
    inventory_icon_.setPosition(camera_coordinates.x + static_cast<float>(camera_size.x) / 2.0f - static_cast<float>(camera_size.x) * 7.5f / 100.0f,
            camera_coordinates.y + static_cast<float>(camera_size.y) - menu_icon_.getGlobalBounds().height - static_cast<float>(camera_size.y) * 1.0f / 100.0f );
    level_points_icon_.setPosition(camera_coordinates.x + static_cast<float>(camera_size.x) / 2.0f + static_cast<float>(camera_size.x) * 7.5f / 100.0f,
            camera_coordinates.y + static_cast<float>(camera_size.y) - menu_icon_.getGlobalBounds().height - static_cast<float>(camera_size.y) * 1.0f / 100.0f );
}
