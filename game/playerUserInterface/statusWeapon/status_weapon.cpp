...
void StatusWeapon::setObjectsPosition(const sf::Vector2u& camera_size) {
    ...
    icon_sprite_.setPosition(zone_.getGlobalBounds().width / 25.0f, zone_.getGlobalBounds().height / 4.0f);
    icon_sprite_.setScale(static_cast<float>(camera_size.x) / 19500.0f,
                        static_cast<float>(camera_size.y) / 10000.0f);
    icon_sprite_.setPosition(zone_.getGlobalBounds().width / 25.0f, zone_.getGlobalBounds().height / 4.0f);
}

void StatusWeapon::setNewWeapon(const std::string& weapon_name, const sf::Vector2u& camera_size) {
    icon_sprite_.setTexture(AssetManager::getInstance()->getTexture(weapon_name));
    icon_sprite_.setScale(static_cast<float>(camera_size.x) / 19500.0f,
                        static_cast<float>(camera_size.y) / 10000.0f);
}
